...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = new URL('/api/finance/reports/export', baseUrl);
  ...
  if (!response.ok) {
    return NextResponse.json({ message: '재정 리포트 CSV를 생성하지 못했습니다.' }, { status: response.status });
  }
  ...
  return new NextResponse(await response.arrayBuffer(), {
    ...
    headers: {
      ...
      'Content-Disposition': response.headers.get('Content-Disposition') ?? 'attachment; filename="finance-report.csv"',
    },
  });
}
